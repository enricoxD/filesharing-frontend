...
import {UploadNotFound} from "@/components/files/fallback/UploadNotFound";
...
export const FilePageContent = ({
  ...
  onPasswordSubmit,
  formData
}: FilePageContentProps) => {
  if (isFetching) return <Loading/>
  if (requiresPassword)
    return <RequestPassword failedTries={failedPasswordTries} onChange={onPasswordChange} onSubmit={onPasswordSubmit}/>
  return upload ?
    <Upload upload={upload} uploadData={formData}/>
    :
    <UploadNotFound/>
}